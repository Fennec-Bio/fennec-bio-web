...
export function VariableImpact() {
  ...
  useEffect(() => {
    ...
    const allSets = [...new Set(groupedData.flatMap(d => d.experiments.flatMap(e => (e.experiment_sets?.length ?? 0) > 0 ? e.experiment_sets! : ['Unknown'])))]
        ...
        g.append('circle').attr('cx', x + bw / 2 + jitter).attr('cy', yScale(exp.titer)).attr('r', 5)
          ...
          .on('mouseover', () => {
            tooltip.style('visibility', 'visible').html(
              `<strong>${exp.title}</strong><br/>${selectedVariable}: ${group.group}<br/>Titer: ${exp.titer.toFixed(1)}<br/><span style="color:#6b7280">${(exp.experiment_sets?.length ?? 0) > 0 ? exp.experiment_sets!.join(', ') : 'No set'}</span>`
            )
          })
          .on('mousemove', (event) => {
